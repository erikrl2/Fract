...
namespace Fract {
	...
	static void updateRes(float ts)
	{
		...
		fData.res = 2.0f / std::min(window.viewportSize.x, window.viewportSize.y) * fData.zoom;
	}
	...
	static void updateStart(float ts)
	{
		...
		if (glfwGetMouseButton(&window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
		{
			...
			vec2 mousePos{ float(xpos), window.viewportSize.y - float(ypos) };
			static vec2 lastMousePos;

			if (pressed)
				startOffset -= (mousePos - lastMousePos) * fData.res;
			else
				pressed = true;
		...
		fData.start = (vec2)window.viewportSize * -0.5f * fData.res + startOffset;
	}
